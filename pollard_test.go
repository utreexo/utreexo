...
func testUndo(t *testing.T, utreexo UtreexoTest) {
	...
	for i, test := range tests {
		...
		case *Forest:
			tmpDir := t.TempDir()
			f, err := newForest(memCached(t, 32), memCached(t, 4), memCached(t, 32), nil, tmpDir+"/ctrl", tmpDir+"/slots", 6, 0)
			if err != nil {
...
func TestModify(t *testing.T) {
	...
	tmpDir := t.TempDir()
	forest, err := newForest(memCached(t, 32), memCached(t, 4), memCached(t, 32), nil, tmpDir+"/ctrl", tmpDir+"/slots", 10, 0)
	if err != nil {
		t.Fatalf("newForest: %v", err)
	}
	testModify(t, forest)
...
func fuzzUndo(t *testing.T, p UtreexoTest, startLeaves uint8, modifyAdds uint8, delCount uint8) {
	...
	case *Forest:
		tmpDir := t.TempDir()
		v, err := newForest(memCached(t, 32), memCached(t, 4), memCached(t, 32), nil, tmpDir+"/ctrl", tmpDir+"/slots", 20, 0)
		if err != nil {
			t.Fatal(err)
		}
		p = v
...
func FuzzUndoChain(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, numAdds, duration uint32, seed int64) {
		...
		case 2:
			tmpDir := t.TempDir()
			p, err := newForest(memCached(t, 32), memCached(t, 4), memCached(t, 32), nil, tmpDir+"/ctrl", tmpDir+"/slots", 17, 0)
			if err != nil {
				t.Fatal(err)
			}
			fuzzUndoChain(t, p, numBlocks, numAdds, duration, seed)
