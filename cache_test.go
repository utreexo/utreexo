...
func cacheStoreFactories(entrySize int, maxBytes int64) []struct {
	name string
	cs   cacheStore
} {
	return []struct {
		...
		{"Mmap", newMmapCacheStore(entrySize, maxBytes)},
	}
}

func TestCacheStorePutGet(t *testing.T) {
	for _, f := range cacheStoreFactories(32, 1<<20) {
...
func TestCacheStorePutGet4(t *testing.T) {
	for _, f := range cacheStoreFactories(4, 1<<20) {
		t.Run(f.name, func(t *testing.T) {
			cs := f.cs
			for i := range 1000 {
				var data [4]byte
...
func TestCacheStorePutGet8(t *testing.T) {
	for _, f := range cacheStoreFactories(8, 1<<20) {
		t.Run(f.name, func(t *testing.T) {
			cs := f.cs
			for i := range 1000 {
				var data [8]byte
...
func TestCacheStoreOverflowed(t *testing.T) {
	...
	for _, f := range cacheStoreFactories(32, 1024) {
		t.Run(f.name, func(t *testing.T) {
			cs := f.cs
			if cs.overflowed() {
...
func TestCacheStoreEntrySize(t *testing.T) {
	for _, es := range []int{4, 8, 32} {
		for _, f := range cacheStoreFactories(es, 1<<20) {
			t.Run(f.name, func(t *testing.T) {
