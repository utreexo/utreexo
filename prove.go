...
func (u *UndoInfo) pop() (*updateHashInfo, *insertDelInfo) {
	uhInfo, idInfo := &updateHashInfo{}, &insertDelInfo{}
...
func calculateHashes2(numLeaves uint64, delHashes []Hash, proof Proof) (
	...
	for row := uint8(0); row <= totalRows; {
		...
		if isRootPositionOnRow(provePos, numLeaves, row) {
			...
			//if modifyHash == empty {
			//	undoInfo.ParentHash = append(undoInfo.ParentHash, empty)
			//	undoInfo.Hashes = append(undoInfo.Hashes, modifySibHash)
			//	undoInfo.IsLeft = append(undoInfo.IsLeft, isLeftNiece(sibling(provePos)))
			//} else {
			//	undoInfo.ParentHash = append(undoInfo.ParentHash, empty)
			//	undoInfo.Hashes = append(undoInfo.Hashes, modifyHash)
			//	undoInfo.IsLeft = append(undoInfo.IsLeft, isLeftNiece(sibling(provePos)))
			//}
			continue
		}
		...
		insI := slices.Index(ingestIns.Hashes, proveHash)
		insJ := slices.Index(ingestIns.Hashes, sibHash)
		if insI != -1 {
