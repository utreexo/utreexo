...
// Clear (for ForEach). [min,max] ranges over each bitmap bound the scan so
// iteration cost stays proportional to live data, not to the virtual
// address space.
package mmapcache
...
//   - dirtyBitmap: dirty — set on Put, cleared by Clear (read by ForEach).
//
