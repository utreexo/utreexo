...
//
// It is safe for concurrent use: multiple goroutines may call Get and Put
// concurrently. Put is fully lock-free — it uses atomic CAS on bitmap
// words and atomic min/max updates for range tracking.
//
// Callers must NOT call ForEach, Clear, ClearDirty, DeleteAbove, or Close
...
//
// Lock-free: uses atomic bitmap load. Safe for concurrent use with Put/Get.
...
//
// Fully lock-free. Uses atomic CAS on both the presence bitmap and the dirty
// bitmap, plus atomic min/max updates for range tracking.
...
func (s *Store) Clear() {
	...
	// Clear dirty bitmap via the coarse hierarchy.
	dlo := int(s.dirtyMinWord.Load())
	dhi := int(s.dirtyMaxWord.Load())
	if dlo <= dhi {
		coarseLo := dlo / bitsPerWord
		coarseHi := dhi / bitsPerWord
